<template>
  ...
  >
    <div class="flex items-start justify-between px-6 py-3 bg-gray-800 border-b border-gray-700">
      <h2 class="text-lg font-semibold text-white">Select Beatmap</h2>
    ...
    <div class="flex-1 overflow-y-auto p-6">
      <template v-if="!lobbyState.currentMappoolId">
        <div class="flex flex-col items-center justify-center">
          ...
          <p class="text-lg text-gray-200 font-medium mb-2">No active mappool selected</p>
          <p class="text-gray-400 mb-4">Please choose a mappool to continue</p>
          <div class="w-full max-w-xs">
            <div class="relative flex items-center">
              ...
              >
                <option disabled value="null">Select a mappool</option>
                <option v-for="pool in mappools" :key="pool.id" :value="pool.id">{{ pool.name }}</option>
              </select>
